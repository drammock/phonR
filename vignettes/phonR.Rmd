...
The vowel polygon lines (and also convex hulls, seen below) are drawn in black color if the `var.col.by` argument is `vowel` (because the polygonal boundary line is a property of the entire *vowel space*, not of any one vowel). If `var.col.by` is something else (like gender or talker) then the polygonal or hull line follows the color coding of the tokens and means. Just like ellipses (and hulls), vowel polygons may also have lines, fills, or both (controlled by the boolean `poly.line` and `poly.fill` arguments).
...
#indo$force[!is.finite(indo$force)] <- with(indo, max(is.finite(force)))
#indo$force <- plotrix::rescale(log(indo$force), c(0, 100))
coolcolors <- plotrix::color.scale(x=0:100, cs1=c(180, 360), cs2=c(25, 75),
                                   cs3=c(50, 100), alpha=1, color.spec='hcl')
indo$force.col <- plotrix::color.scale(indo$force, cs1=c(180, 360), cs2=c(25, 75),
                                       cs3=c(25, 75), alpha=0.75, color.spec='hcl')
par(mfrow=c(2, 2))
with(indo, plotVowels(f1, f2, vowel, plot.tokens=TRUE, pch.tokens=vowel, alpha.tokens=0.4,
                      ...
                      col=force.col, pretty=TRUE, var.col.by=NA))
with(indo, plotVowels(f1, f2, vowel, plot.tokens=TRUE, pch.tokens=vowel, alpha.tokens=0.4,
