...
cdist <- with(indo, apply(cbind(f2, f1), 1, function(i) dist(rbind(i, colMeans(cbind(f2, f1))))))
...
### The `pretty` argument
By now you will have noticed that the default colors and symbols are ugly, and the axes are on the wrong sides of the plot (from a phonetician’s perspective, anyway). `plotVowels` has a wonderful argument called `pretty` that sets a whole bunch of default color and style options all at once, as a shortcut to getting plots that are easy on the eyes. Here are the same four plots as above, showing the uses of `var.col.by` and `var.style.by`, but this time they are `pretty`:
...
with(indo, plotVowels(f1, f2, vowel, plot.tokens=TRUE, pch.tokens=vowel,
                      ...
                      var.col.by=vowel, family="Charis SIL", pretty=TRUE))
```
...
Although seeing the cloud of individual tokens for each vowel is sometimes informative, the plot may become hard to read if there is too much overlap (e.g., due to plotting several different talkers on the same graph). The ability to specify `alpha.tokens` mitigates this to some extent, but in some cases even this is not enough and the plot becomes unreadable. In such cases, ellipses summarizing the range of variation of each vowel can provide a convenient summary of the underlying data with less visual clutter. The next plot shows (in the top row) two plots showing the effect of the `ellipse.conf` argument. The default is `ellipse.conf=0.6827`, which is an α-level corresponding to ±1 standard deviation from the bivariate mean. The upper right-hand plot shows `ellipse.conf=0.95`, which gives 95% confidence ellipses. The individual tokens are left visible to show their relationship with the ellipse extents. The bottom row shows how ellipses can be drawn as lines, fills, or both; how plotting ellipses interacts with the `group` argument; how the `var.style.by` argument affects the ellipse lines; and the utility of reducing the `fill.opacity` argument when lots of filled objects overlap.
...
```{r, fig.cap="Color-coding repulsive force using individual tokens, versus plotting a heatmap"}
...
with(indo, plotVowels(f1, f2, vowel, plot.tokens=FALSE, force.heatmap=TRUE,
                      ...
                      force.legend=TRUE, pretty=TRUE))
                      #force.colmap=coolcolors, force.res=10, pretty=TRUE))
```

The resolution of the heatmap is controlled by the `resolution` argument, which must be passed as part of the named list `heatmap.args`. The colormap used to draw the heatmap defaults to grayscale if `pretty=FALSE` or to a cool palette if `pretty=TRUE`, but can be specified by passing a vector of color values as the `colormap` item in `heatmap.args`. The plots below illustrate the effect of increasing `resolution` using just one subject's vowels. 
...
force.col <- plotrix::color.scale(force, cs1=c(150, 330), cs2=c(25, 75),
                                  cs3=c(60, 90), color.spec='hcl')
with(female2, plotVowels(f1, f2, vowel, pch.tokens=vowel, col=force.col, 
...
These plots show that the boundary between the vowels [e] and [a] for this speaker is somewhat crowded, as is the boundary between [o] and [u], and to a lesser extent so is the boundary between [i] and [e]. The plots also show that there can be a “sweet spot” when choosing resolution that makes these characteristics most readily visible; making the resolution too high shrinks the size of the pixels and thereby makes the regions of high force smaller and harder to see.

In theory, resolution can go as high as you want, but the heatmaps are somewhat computationally expensive to generate so it is often best to keep the resolution low during “exploratory” plotting, and only increase the resolution when you are ready to generate plots for use in finished documents. Alternatively, you can pass `fast=TRUE` as part of the `heatmap.args` list, which will use an interpolation algorithm to assign force values to the pixels, rather than calculating the actual force value at that pixel as if there were a vowel there.

```{r, fig.cap="Comparison of `fast` options for heatmaps. Left: TRUE. Right: FALSE."}
par(mfrow=c(1, 2))
for (speed in c(TRUE, FALSE)) {
    with(female2, plotVowels(f1, f2, vowel, plot.tokens=FALSE, force.heatmap=TRUE,
                             heatmap.args=list(colormap=coolcolors, resolution=60,
...
with(indo, plotVowels(cbind(f1, f1a, f1b, f1c, f1d), cbind(f2, f2a, f2b, f2c, f2d), 
                      vowel, plot.tokens=TRUE, pch.tokens=NA, alpha.tokens=0.2,
                      plot.means=TRUE, pch.means=vowel, cex.means=2, var.col.by=vowel,
                      pretty=TRUE,  diph.arrows=TRUE, diph.args.tokens=list(lwd=0.75),
                      diph.args.means=list(lwd=4)))
```
