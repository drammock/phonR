...
```{r setup-options, echo=FALSE, cache=FALSE, include=FALSE, fig.width=12, fig.height=12}
...
```{r convex-hull, dependson=c("load-phonR"), fig.height=6, fig.cap="Convex hulls with different values for the `color.by` argument"}
par(mfrow=c(1, 2))
...
```{r force-fast, dependson=c("load-phonR", "force-one"), fig.height=6, fig.cap="Comparison of `fast` options for heatmaps. Left: `fast=TRUE`. Right: `fast=FALSE`.", cache=TRUE}
par(mfrow=c(1, 2))
