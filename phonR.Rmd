...
```{r, fig.width=3.75, fig.height=3.75, fig.cap="A minimal `plotVowels` call"}
...
```{r, fig.width=3.75, fig.height=3.75, fig.cap="Specifying `vowel` and `group` doesn’t automatically change the appearance!"}
...
```{r, fig.cap="Four different combinations of `var.col.by` and `var.style.by`"}
par(mfrow=c(2, 2))
...
```{r, fig.height=3.75, fig.cap="Using `col` and `var.col.by=NA` to specify unique colors for each data point."}
par(mfrow=c(1, 2))
...
```{r, fig.height=3.75, fig.cap="Mean F1 and F2 values for each vowel, plotted with different grouping factors"}
par(mfrow=c(1, 2))
...
with(indo, plotVowels(f1, f2, unicodevowel, plot.tokens=TRUE, pch.tokens=vowel,
                      ...
                      pch.means=unicodevowel, cex.means=4, var.col.by=unicodevowel, 
                      family="Charis SIL", pretty=TRUE))
```
...
```{r, fig.cap="Ellipses in various configurations"}
par(mfrow=c(2, 2))
...
```{r, fig.width=3.75, fig.height=3.75, fig.cap="The “vowel means” polygon"}
...
```{r, fig.height=3.75, fig.cap="Color-coding repulsive force using individual tokens, versus plotting a heatmap", cache=TRUE}
force <- with(indo, repulsiveForce(f2, f1, vowel))
force.col <- plotrix::color.scale(force, cs1=c(150, 330), cs2=c(25, 75),
...
with(indo, plotVowels(f1, f2, vowel, pch.tokens=vowel, col=force.col, var.col.by=NA,
                      pretty=TRUE))
with(indo, plotVowels(f1, f2, vowel, plot.tokens=FALSE, heatmap=TRUE,
...
```{r, fig.cap="Repulsive force heatmaps of increasing resolution", cache=TRUE}
par(mfrow=c(2, 2))
...
```{r, fig.height=3.75, fig.cap="Comparison of `fast` options for heatmaps. Left: `fast=TRUE`. Right: `fast=FALSE`.", cache=TRUE}
par(mfrow=c(1, 2))
...
```{r, fig.cap="Illustration of automatic legend creation."}
par(mfrow=c(2, 2))
...
```{r, fig.cap="Plotting multiple timepoints for each vowel."}
