...
output:
  ...
  pdf_document:
    ...
    pandoc_args: ["--no-tex-ligatures", "--natbib"]
---
```{r setup-options, echo=FALSE, cache=FALSE, include=FALSE}
knitr::opts_chunk$set(collapse=TRUE, comment="#>", warning=FALSE, message=FALSE, tidy=TRUE, tidy.opts=list(blank=FALSE, width.cutoff=60), fig.path="images/", dev="CairoPNG")
```
...
with(indo, plotVowels(f1, f2, vowel, plot.tokens=FALSE, heatmap=TRUE, heatmap.args=list(colormap=coolcolors, resolution=10), heatmap.legend=TRUE, pretty=TRUE))
```
...
for (res in c(20, 40, 60)) {
    with(female2, plotVowels(f1, f2, vowel, plot.tokens=FALSE, heatmap=TRUE, heatmap.args=list(colormap=coolcolors, resolution=res), pretty=TRUE))
}
```
...
for (speed in c(TRUE, FALSE)) {
    time.st <- Sys.time()
    with(female2, plotVowels(f1, f2, vowel, plot.tokens=FALSE, heatmap=TRUE, heatmap.args=list(colormap=coolcolors, resolution=50, fast=speed), pretty=TRUE))
    time.nd <- Sys.time()
